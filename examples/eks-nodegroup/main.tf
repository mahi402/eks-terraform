...
module "eks" {
  ...
  parameter_cidr1                 = var.parameter_cidr1
  parameter_cidr2                 = var.parameter_cidr2
  parameter_cidr3                 = var.parameter_cidr3
...
module "eks_kubernetes_addons" {
  ...
  enable_opa_templates             = var.enable_opa_templates
  enable_opa_constraints           = var.enable_opa_constraints
...
module "aws_eks_rbac" {
  ...
  map_roles = [
    {
      rolearn  = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:group/TekAdmins"
      username = "Cloudadmin"
      groups   = ["system:masters"]
    },
    {
      rolearn  = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:user/mahender.tirumala@tekyantra.com"
      username = "Superadmin"
      groups   = ["system:masters"]
    },
    {
...
######Calling INdividual modules after creating cluster
...
module "cloudwatchalarm" {
  ...
  log_group_name                      = "/sukx/application-logs/"
