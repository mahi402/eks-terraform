...
module "opa_gatekeeper" {
  ...
  source        = "../opa"
  helm_config   = var.opa_gatekeeper_helm_config
  addon_context = local.addon_context
}

module "opa_templates" {
  depends_on    = [module.opa_gatekeeper]
  count         = var.enable_opa_templates ? 1 : 0
  source        = "../opa-templates"
  helm_config   = var.opa_templates_helm_config
  addon_context = local.addon_context

}

module "opa_constraints" {
  depends_on    = [module.opa_gatekeeper, module.opa_templates]
  count         = var.enable_opa_constraints ? 1 : 0
  source        = "../opa-constraints"
  helm_config   = var.opa_constraints_helm_config
  addon_context = local.addon_context
